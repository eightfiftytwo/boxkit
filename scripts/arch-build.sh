...
useradd -m -G wheel yayay && passwd -d yayay
chown -R yayay:yayay /tmp/yay-build
echo 'yayay ALL=(ALL) NOPASSWD: ALL' >> /etc/sudoers
su - yayay -c "git clone https://aur.archlinux.org/yay-bin.git /tmp/yay-build/yay"
